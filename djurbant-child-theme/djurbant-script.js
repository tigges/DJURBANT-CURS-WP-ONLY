...
const FALLBACK_YOUTUBE_URL = "/video/";
...
const GENRE_PATTERNS = Object.freeze([
  [/\bvocal\s*deep\s*house\b/i, "vocal-deep-house"],
  [/\bbass[\s-]*house\b/i, "bass-house"],
  [/\btech[\s-]*house\b/i, "tech-house"],
  [/\bdeep[\s-]*house\b/i, "deep-house"],
  [/\blo[\s-]*fi[\s-]*house\b/i, "lo-fi-house"],
  [/\bjazz[\s-]*house\b/i, "jazz-house"],
  [/\blatin[\s-]*house\b/i, "latin-house"],
  [/\bfrench[\s-]*house\b/i, "french-house"],
  [/\bdisco[\s-]*house\b/i, "disco-house"],
  [/\bafro[\s-]*house\b/i, "afro-house"],
  [/\bamapiano\b/i, "amapiano"],
  [/\bgqom\b/i, "gqom"],
  [/\bukg\b/i, "uk-garage"],
  [/\buk[\s-]*garage\b/i, "uk-garage"],
  [/\bbaile[\s-]*funk\b/i, "baile-funk"],
  [/\bdrift[\s-]*phonk\b/i, "drift-phonk"],
...
function loadOfflineAudioSources() {
  ...
  offlineAudioSourcesPromise = fetch((window.__DJURBANT_THEME_URI || ".") + "/offline-media/audio-sources.json", { cache: "no-store" })
    .then((response) => {
      if (!response.ok) {
        return { tracks: {} };
      }
...
function bindMainNavLiveClick() {
  ...
  navLiveLinks.forEach((link) => {
    ...
    link.addEventListener("click", (event) => {
      ...
      event.preventDefault();
      window.location.href = "/?autoplayTopVideo=1#best-of-artist";
    });
  });
}
  ...
  navLiveLinks.forEach((link) => {
    ...
    link.href =
      youtubeLive?.isLive && targetUrl
        ...
        : page === "home"
          ? "#best-of-artist"
          : "/?autoplayTopVideo=1#best-of-artist";
...
async function hydrateMediaWalls() {
  ...
  try {
    const response = await fetch((window.__DJURBANT_THEME_URI || ".") + "/media-data.json", { cache: "no-store" });
    if (!response.ok) {
      return;
    }
...
function bindFallbackMainNavLiveLink() {
  ...
  navLiveLinks.forEach((link) => {
    ...
    link.href = page === "home" ? "#best-of-artist" : "/#best-of-artist";
    link.removeAttribute("target");
    link.removeAttribute("rel");
