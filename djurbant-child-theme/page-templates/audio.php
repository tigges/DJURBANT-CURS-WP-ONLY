<?php
/**
 * Template Name: DJ UrbanT Audio
 *
 * Renders the original djurbant.com audio sub-page inside WordPress.
 */
...
<head>
    ...
    <style>
      body[data-page="audio-more"] .media-section { padding-top: var(--space-3); margin-top: var(--space-4); }
      body[data-page="audio-more"] .subpage-grid .media-card.chunk-hidden { display: none !important; }
      body[data-page="audio-more"] .subpage-grid .media-card.chunk-force-visible[hidden] { display: flex !important; }
      body[data-page="audio-more"] .subpage-grid .chunk-load-more-tile { display: flex; align-items: stretch; }
      body[data-page="audio-more"] .subpage-grid .chunk-load-more-inner { flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 0.6rem; border-radius: 8px; border: 1px solid rgba(76, 196, 255, 0.38); background: rgba(7, 12, 26, 0.9); box-shadow: inset 0 0 0 1px rgba(255,255,255,0.05), 0 8px 24px rgba(0,0,0,0.3); padding: 1rem; }
      body[data-page="audio-more"] .subpage-grid .chunk-load-more-btn { width: 100%; }
      body[data-page="audio-more"] .subpage-grid .chunk-load-more-hint { margin: 0; font-size: 0.78rem; letter-spacing: 0.04em; color: rgba(220,226,240,0.68); }
    </style>
</head>
<body data-page="audio-more" <?php body_class(); ?>>
<?php wp_body_open(); ?>
    ...
    <main class="subpage-main">
      <section class="media-section">
        <div class="section-head">
          <h1 data-cms-text="page.title">Audio</h1>
        </div>
        <p class="subpage-intro" data-cms-text="page.intro">Ranked from fetched audio channel stats.</p>
        <div id="audio-rest-grid" class="subpage-grid"></div>
        <div class="section-cta">
          <a class="btn btn-outline" href="<?php echo home_url('/?bestOf=audio#best-of-artist'); ?>">Top Audio</a>
        </div>
      </section>
    </main>
...
<?php // Inline chunked loading script for the audio subpage grid ?>
<script>
(() => {
    ...
    let att = 0; const iv = setInterval(() => { att++; const c = document.getElementById("audio-rest-grid"); if (!c) { if (att >= 160) clearInterval(iv); return; } if (c.dataset.chunkInit === "1") { clearInterval(iv); return; } if (initChunkedContainer("audio-rest-grid")) { c.dataset.chunkInit = "1"; clearInterval(iv); } }, 120);
    window.addEventListener("resize", () => { const c = document.getElementById("audio-rest-grid"); c?._applyChunkState?.(); }, { passive: true });
})();
</script>
